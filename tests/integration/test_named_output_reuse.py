...
from proofsignal_spec.workspace.repository import publish_named_outputs, resolve_named_output
...
def test_later_use_case_resolves_published_named_output(tmp_path) -> None:
        ...
        [
            {
                ...
                "value": "https://app.example.test/project/proofsignal-collab-abc123",
    ...
    assert output["value"].endswith("/proofsignal-collab-abc123")
...
def test_run_resolves_named_output_runtime_input_before_core(tmp_path, monkeypatch) -> None:
    ...
    (tmp_path / ".proofsignal/run-requests/project-details.yaml").write_text(
        json.dumps(
            {
    ...
    (tmp_path / ".proofsignal/skills/project-details.browser.md").write_text(
        """# Project details
...
""",
    ...
    save_use_case(
        ...
        UseCaseRecord(
            ...
            runRequest=ArtifactReference(path=".proofsignal/run-requests/project-details.yaml", kind="run-request", id="request.project-details", version="1.0.0"),
            mainSkill=ArtifactReference(path=".proofsignal/skills/project-details.browser.md", kind="skill", id="skill.project-details", version="1.0.0"),
            skills=[ArtifactReference(path=".proofsignal/skills/project-details.browser.md", kind="skill", id="skill.project-details", version="1.0.0")],
    ...
    prepared = list((tmp_path / ".proofsignal/runs/project-details").glob("*.run-request.json"))
    assert prepared
    params = json.loads(prepared[0].read_text(encoding="utf-8"))["parameters"]
    assert params["createdProjectUrl"] == "https://app.example.test/project/proofsignal-collab-abc123"
