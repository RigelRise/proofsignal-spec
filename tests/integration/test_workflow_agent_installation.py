...
from proofsignal_spec.integrations.manifests import save_manifest, sha256_text
from proofsignal_spec.workspace.models import AgentIntegrationState, ManagedFileRecord
...
class WorkflowAgentInstallationTests(CliTestCase):
    ...
    def test_claude_upgrade_removes_previous_managed_legacy_skills(self) -> None:
        legacy_path = ".claude/skills/proofsignal-spec-author/SKILL.md"
        ...
        save_manifest(
            ...
            AgentIntegrationState(
                ...
                managedFiles=[ManagedFileRecord(path=legacy_path, sha256=sha256_text(legacy_content), source="claude/proofsignal-spec-author", kind="agent-skill")],
            ),
        )
