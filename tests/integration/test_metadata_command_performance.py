...
def _create_many_metadata_only_use_cases(project, *, count: int) -> None:
    ...
    for index in range(count):
        ...
        (project / f".proofsignal/run-requests/{alias}.yaml").write_text("{}", encoding="utf-8")
        (project / f".proofsignal/skills/{alias}.browser.md").write_text("# skill\n", encoding="utf-8")
        save_use_case(
            project,
            UseCaseRecord(
                alias=alias,
                ...
                mainSkill=ArtifactReference(path=f".proofsignal/skills/{alias}.browser.md", kind="skill"),
                skills=[ArtifactReference(path=f".proofsignal/skills/{alias}.browser.md", kind="skill")],
