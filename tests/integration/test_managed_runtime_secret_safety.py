...
class ManagedRuntimeSecretSafetyTests(CliTestCase):
    def test_init_output_and_project_state_do_not_contain_email_token_receipt_or_signed_url(self) -> None:
        ...
        os.environ["VERIFYSIGNAL_EMAIL"] = "person@example.com"
        os.environ["VERIFYSIGNAL_CORE_VERSION"] = "0.5.1"
        self.addCleanup(lambda: os.environ.pop("VERIFYSIGNAL_RUNTIME_CACHE_DIR", None))
        self.addCleanup(lambda: os.environ.pop("VERIFYSIGNAL_EMAIL_UNLOCK_TOKEN", None))
        self.addCleanup(lambda: os.environ.pop("VERIFYSIGNAL_EMAIL", None))
        self.addCleanup(lambda: os.environ.pop("VERIFYSIGNAL_API_BASE_URL", None))
        self.addCleanup(lambda: os.environ.pop("VERIFYSIGNAL_CORE_VERSION", None))
