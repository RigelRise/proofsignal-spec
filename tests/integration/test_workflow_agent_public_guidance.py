...
def test_codex_and_claude_guidance_describes_first_run_stage_cards(tmp_path) -> None:
    ...
    relevant = "\n".join(content for path, content in files.items() if "verifysignal-run" in path or "verifysignal-spec-run" in path or path in {"AGENTS.md", "CLAUDE.md"})
