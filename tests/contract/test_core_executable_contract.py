...
def test_core_adapter_invokes_public_contracts_operation() -> None:
    ...
    assert result["schema"] == "verifysignal.contracts/v1"
...
def test_core_contract_projection_accepts_declared_multi_skill_support() -> None:
    ...
    os.environ["FAKE_VERIFYSIGNAL_MODE"] = "multi-skill-supported"
    try:
        raw = CoreAdapter(executable=str(FAKE_CORE)).contracts()
    finally:
        if old_mode is None:
...
def test_core_contract_projection_filters_experimental_items() -> None:
    ...
    old_mode = os.environ.get("FAKE_VERIFYSIGNAL_MODE")
    os.environ["FAKE_VERIFYSIGNAL_MODE"] = "experimental-contract"
    try:
        raw = CoreAdapter(executable=str(FAKE_CORE)).contracts()
    finally:
        if old_mode is None:
