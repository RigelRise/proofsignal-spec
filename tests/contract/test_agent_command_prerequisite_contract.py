...
def test_specify_template_requires_prerequisite_check_and_understanding_guidance(tmp_path) -> None:
    ...
    for path in [
        ...
        content = files[path]
        assert "proofsignal-spec workflow check specify --json" in content
        assert "Do not use `npx proofsignal-spec`" in content
        assert "If `workflow check` is unavailable" in content
        assert "Do not fall back to `proofsignal-spec check`, directory listing, repository inspection, or use-case questions" in content
...
def test_later_stage_templates_use_workflow_check_before_stage_work(tmp_path) -> None:
    files = _rendered_workflow_files(tmp_path)
    expected = {
        "clarify": "proofsignal-spec workflow check clarify --alias <alias> --json",
        "plan": "proofsignal-spec workflow check plan --alias <alias> --json",
        "tasks": "proofsignal-spec workflow check tasks --alias <alias> --json",
        "implement": "proofsignal-spec workflow check implement --alias <alias> --json",
        "validate": "proofsignal-spec workflow check validate --alias <alias> --json",
        "run": "proofsignal-spec workflow check run --alias <alias> --json",
        "repair": "proofsignal-spec workflow check repair --alias <alias> --json",
    }
...
def test_understand_and_list_templates_document_no_prerequisite_behavior(tmp_path) -> None:
    ...
    assert "proofsignal-spec workflow check understand --json" in files[".agents/skills/proofsignal-understand/SKILL.md"]
