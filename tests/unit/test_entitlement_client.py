...
from verifysignal_spec.runtime.entitlement import DEFAULT_API_BASE_URL, EntitlementClient, resolve_entitlement_config
from verifysignal_spec.runtime.models import redact_runtime_payload
...
def test_entitlement_config_defaults_to_production_and_reports_override_sources(monkeypatch) -> None:
    ...
    monkeypatch.setenv("VERIFYSIGNAL_API_BASE_URL", "http://127.0.0.1:9999/api")
...
def test_entitlement_client_validates_exchange_schema_and_public_free_policy() -> None:
    with serve_fake_entitlement_backend() as (api_base_url, _state):
        exchange = EntitlementClient(resolve_entitlement_config(api_base_url=api_base_url)).exchange_token("vs_valid")
...
def test_entitlement_redaction_removes_email_token_receipt_and_backend_details() -> None:
    ...
    assert "vs_secret_token" not in text
