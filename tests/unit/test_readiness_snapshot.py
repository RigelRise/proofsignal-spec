...
from verifysignal_spec.workspace.repository import load_use_case, readiness_current_state
...
def test_artifact_change_invalidates_snapshot(tmp_path) -> None:
    ...
    (tmp_path / ".verifysignal/skills/about-page-unauth.browser.md").write_text("changed", encoding="utf-8")
    record = load_use_case(tmp_path, "about-page-unauth")
...
def _save_snapshot_with_spec_version(tmp_path, alias: str, spec_version: str) -> None:
    ...
    from verifysignal_spec.workspace.repository import artifact_fingerprints, save_readiness_snapshot
...
def test_committed_write_with_matching_supersede_review_is_confirmed(tmp_path) -> None:
    ...
    from verifysignal_spec.workspace.repository import save_supersede_review, save_use_case
...
def test_stale_and_needs_validate_labels_are_distinct() -> None:
    ...
    from verifysignal_spec.workspace.repository import _current_label
...
def test_next_action_present_iff_state_is_actionable() -> None:
    ...
    from verifysignal_spec.workspace.repository import _readiness_next_action, _readiness_presentation
...
def test_patch_bump_keeps_readiness_minor_stable() -> None:
    ...
    from verifysignal_spec.workspace.repository import _spec_minor
