...
from proofsignal_spec.runtime.entitlement import load_receipt, receipt_path, receipt_status, resolve_entitlement_config, save_receipt
from proofsignal_spec.runtime.entitlement import EntitlementClient
...
def test_receipt_storage_uses_user_cache_permissions_and_summary(tmp_path: Path, monkeypatch) -> None:
    ...
    with serve_fake_entitlement_backend() as (api_base_url, _state):
        receipt = EntitlementClient(resolve_entitlement_config(api_base_url=api_base_url)).exchange_token("ps_valid").receipt
    ...
    assert path.read_text(encoding="utf-8").startswith('{"schema":"proofsignal.entitlement-receipt/v1"')
