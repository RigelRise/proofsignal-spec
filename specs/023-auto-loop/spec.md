# Feature Specification: Autonomous Grounded Authoring Loop (`/proofsignal-auto`)
...
- **Project-local workspace portability**: The loop persists artifacts only through the existing `workflow persist` path into `.proofsignal/`; it introduces no new global state. Existing deterministic, non-AI flows (`author`/`validate`/`run`) remain usable unchanged.
...
- **Agent-neutral interface**: `/proofsignal-auto` is an additive AI-only convenience installed as an adapter over the shared workspace + CLI contract, rendered identically for Claude and Codex. It orchestrates existing stages; it does not replace the deterministic CLI, and registered use cases remain listable/runnable without AI.
...
As a developer using Claude Code or Codex, I want a single `/proofsignal-auto "<goal> against <url>"` command that drafts a browser use case from my source, grounds its selectors against the live app, applies the grounded corrections, validates, runs, and (when safe) auto-repairs — stopping only when it genuinely needs my input — so I reach a green validation in one conversation instead of stepping through nine stages.

**Why this priority**: This is the entire value of the feature and the smallest demonstrable slice.

**Independent Test**: Run `/proofsignal-auto` against the Core `discover-grounding-app`; the agent drafts targets, `discover` corrects the divergent ones, the use case persists, validates, and `run` reports passed — with at most a credential prompt as a stop.
...
As a user whose installed Core predates dynamic grounding, I want `/proofsignal-auto` to detect that `discover` is unavailable and either tell me to upgrade or continue source-only, rather than fail opaquely.
...
- The deterministic non-AI path (`proofsignal run <alias>`) must keep working unchanged when `/proofsignal-auto` is not used.
...
- **FR-002**: Spec MUST provide `core_supports_discover(version_response)` that returns true only when the `version` operations array contains a `discover` entry with schema `proofsignal.discover/v1`; it MUST NOT be added to `REQUIRED_OPERATIONS`.
- **FR-003**: An installed `/proofsignal-auto` command MUST be rendered for both Claude and Codex from a single template and MUST drive: confirm capability → draft from source → ground via `discover` → apply corrections → persist via `workflow persist` → validate → run → bounded safe auto-repair.
...
- **FR-005**: The loop MUST persist artifacts only through `workflow persist`; it MUST NOT write managed `.proofsignal/` artifacts directly and MUST NOT persist credential values.
- **FR-006**: `/proofsignal-auto` MUST be additive: existing staged commands, `WORKFLOW_STAGES`, and the deterministic non-AI path MUST remain unchanged (`auto` is an orchestration command, not a persistable stage).
...
- **Discover capability**: derived from the Core `version` operations array; optional, schema `proofsignal.discover/v1`.
...
- **SC-001**: Against the Core `discover-grounding-app`, `/proofsignal-auto` produces one grounded green run with at most one user stop (credentials).
...
- **SC-004**: Installing the Claude or Codex integration produces a `proofsignal-auto/SKILL.md`.

## Assumptions

- Core 016 (dynamic grounding `discover`) is available on a current Core; `/proofsignal-auto` degrades gracefully otherwise.
