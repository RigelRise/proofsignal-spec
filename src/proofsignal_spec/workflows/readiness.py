...
def core_readiness(project: Path, core_cmd: str | None = None) -> CoreReadiness:
    ...
    message = runtime.message or (blocker.message if blocker else "ProofSignal Core failed readiness check.")
    ...
    if blocker_code == "core.missing":
        return CoreReadiness(
            ...
            requiredOperations=contract["requiredOperations"],
            message=(
                f"{message} Structural workspace validation can still run, but ProofSignal Core is required "
                "for the complete ProofSignal validation and browser execution experience."
            ),
        )
    ...
    if not core_command and core_contract is None:
        return [
            ReadinessBlocker(
                ...
                message="ProofSignal Core command is unavailable, so the public executable contract cannot be discovered.",
                repairable=False,
...
def managed_runtime_contract_blockers(runtime: Any) -> list[ReadinessBlocker]:
    ...
    return [
        ReadinessBlocker(
            code="core-contract.bootstrap-incompatible",
            message="ProofSignal Core does not advertise the required public `contracts` operation.",
            repairable=False,
            documentationRef="coreExecutableContract",
        )
    ]
