...
def write_specification(project: Path, alias: str, goal: str, runtime_assumptions: list[str] | None = None) -> str:
    return write_markdown(
        ...
        f"Use Case Specification: {alias}",
        {
            ...
            "Write-Flow Safety": "Declare sideEffects.class explicitly. Write and external-notification cases can be drafted here, but executable readiness requires Core sideEffectGuardrails, commitStepId, a local envelope, and rerunPolicy.",
...
def write_artifact_plan(project: Path, plan: ArtifactPlan) -> str:
    ...
    return write_markdown(
        ...
        f"Artifact Plan: {plan.useCaseAlias}",
        {
            ...
            "Write-Flow Safety": "For write/external-notification plans, include sideEffects, runtimeOutputs when needed, and rerunPolicy in the stage payload. Draft plans do not guarantee executable readiness until validation checks Core guardrails.",
