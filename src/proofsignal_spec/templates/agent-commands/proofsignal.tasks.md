...
- Start by running `proofsignal workflow check tasks --alias <alias> --json`.
- Before constructing the payload, read the public workflow contract with `proofsignal workflow info proofsignal-use-case --json` and use `stagePayloadContracts.tasks` as the source of truth. Do not inspect installed package source to infer payload schemas.
...
- Read the approved plan and persisted context with `proofsignal workflow show --alias <alias> --json`.
...
- Do not write managed `.proofsignal/` artifacts directly. Persist managed artifacts through `proofsignal workflow persist tasks --alias <alias> --payload <payload.json> --json`.
