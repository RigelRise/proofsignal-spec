...
- Start by running `verifysignal workflow check plan --alias <alias> --json`.
- Before constructing the payload, read the public workflow contract with `verifysignal workflow info verifysignal-use-case --json` and use `stagePayloadContracts.plan` as the source of truth. Use `coreExecutableContract` for Core-owned executable artifact/report sections. Do not inspect installed package source to infer payload schemas.
...
- Read existing persisted context with `verifysignal workflow show --alias <alias> --json`. Do not invent `workflow show` alternatives or use `workflow status` as a use-case reader.
- Stop when unresolved runtime, data, credential, permission, or expected-outcome clarifications remain. Route back to `/verifysignal-clarify`.
...
- Plan skills as decoupled reusable artifacts under `.verifysignal/skills/<name>.browser.md`; one skill may be referenced by multiple run requests.
...
- Do not write managed `.verifysignal/` artifacts directly. Persist managed artifacts through `verifysignal workflow persist plan --alias <alias> --payload <payload.json> --json`.
- Block implementation when the plan lacks a run request or skill relationship.
- Suggest `/verifysignal-tasks` after the plan is approved.
